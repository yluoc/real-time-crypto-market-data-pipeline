...
class StdoutSink(Sink):
    ...
    async def write(self, event: NormalizedEvent) -> None:
        ...
        # Also compute in microseconds with 3 decimals for reference
        lat_recv_to_decode_us = lat_recv_to_decode_ns / 1000.0
        lat_decode_to_proc_us = lat_decode_to_proc_ns / 1000.0
        ...
        if isinstance(event.payload, BookPayload):
            ...
            print(
                ...
                f"Ex->Recv={lat_ex_to_recv_ms}ms Recv->Decode={lat_recv_to_decode_ns}ns ({lat_recv_to_decode_us:.3f}us) Decode->Proc={lat_decode_to_proc_ns}ns ({lat_decode_to_proc_us:.3f}us)"
            )
